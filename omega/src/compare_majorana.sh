...
make -j $jobs -C $build/omega/bin \
   omega_SM.opt omega_SM_Majorana.opt || exit 1

omega_dirac="$build/omega/bin/omega_SM.opt -target:width $width"
omega_majorana="$build/omega/bin/omega_SM_Majorana.opt -target:width $width"
