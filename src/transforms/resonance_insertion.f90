...
contains
  ...
  subroutine evt_resonance_write (evt, unit, verbose, more_verbose, testflag)
    ...
    call evt%res_history_set%write (u, indent=1)
    if (allocated (evt%instance)) then
       write (u, "(3x,A)")  "Subprocess instances: allocated"
    else
       write (u, "(3x,A)")  "Subprocess instances: not allocated"
    end if
    if (evt%particle_set_exists) then
       if (evt%selected_history > 0) then
          write (u, "(3x,A,I0)")  "Selected: resonance history #", &
               evt%selected_history
       else
          write (u, "(3x,A)")  "Selected: no resonance history"
       end if
    else
       write (u, "(3x,A)")  "Selected: [none]"
    end if
    write (u, "(3x,A,1x," // FMT_12 // ")")  &
  ...
  subroutine evt_resonance_determine_on_shell_histories (evt, index_array)
    class(evt_resonance_t), intent(in) :: evt
    integer, dimension(:), allocatable, intent(out) :: index_array
    call evt%res_history_set%determine_on_shell_histories &
         (evt%particle_set%get_outgoing_momenta (), &
         evt%on_shell_limit, index_array)
  ...
  subroutine evt_resonance_compute_probabilities (evt)
    ...
    call evt%evaluate_subprocess (index_array)
    allocate (sqme_res (n), source = 0._default)
    call evt%get_subprocess_sqme (sqme_res, index_array)
  ...
  subroutine evt_resonance_make_particle_set &
    ...
    integer :: n_beam, n_in, n_vir, n_res, n_out, i, i_res, i_term
    ...
    if (associated (evt%previous)) then
       if (evt%previous%particle_set_exists) then
          if (evt%selected_history > 0) then
             ...
             call evt%res_history_set%get_tree (evt%selected_history, res_tree)
