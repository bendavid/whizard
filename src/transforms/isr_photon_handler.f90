...
module isr_photon_handler
  ...
  type, extends (evt_t) :: evt_isr_t
     ...
     integer, dimension(2) :: i_photon = 0
     type(particle_t), dimension(2) :: photon
   ...
   contains
     procedure :: get_mode_string => evt_isr_get_mode_string
     procedure :: set_mode_string => evt_isr_set_mode_string
     procedure :: write_name => evt_isr_write_name
     procedure :: write_mode => evt_isr_write_mode
     procedure :: write_data => evt_isr_write_data
     procedure :: write => evt_isr_write
     procedure :: import_rng => evt_isr_import_rng
     procedure :: set_data => evt_isr_set_data
     procedure, private :: identify_photons
     ...
     procedure, private :: replace_photons
     procedure, private :: replace_partons
     procedure :: transform_outgoing => evt_transform_outgoing
     procedure :: generate_weighted => &
          evt_isr_generate_weighted
     procedure :: make_particle_set => &
          evt_isr_make_particle_set
     procedure :: prepare_new_event => &
          evt_isr_prepare_new_event
  end type evt_isr_t


contains

  function evt_isr_get_mode_string (evt) result (string)
    type(string_t) :: string
    class(evt_isr_t), intent(in) :: evt
    select case (evt%mode)
    case (ISR_TRIVIAL_COLLINEAR)
       string = "trivial, collinear"
    case (ISR_PAIR_RECOIL)
       string = "photon-pair recoil"
    case default
       string = "[undefined]"
    end select
  end function evt_isr_get_mode_string

  subroutine evt_isr_set_mode_string (evt, string)
    class(evt_isr_t), intent(inout) :: evt
  ...
  end subroutine evt_isr_set_mode_string

  subroutine evt_isr_write_name (evt, unit)
    class(evt_isr_t), intent(in) :: evt
    integer, intent(in), optional :: unit
    integer :: u
    u = given_output_unit (unit)
    write (u, "(1x,A)")  "Event transform: ISR photon handler"
  end subroutine evt_isr_write_name

  subroutine evt_isr_write_mode (evt, unit)
    class(evt_isr_t), intent(in) :: evt
    integer, intent(in), optional :: unit
    integer :: u
    u = given_output_unit (unit)
    write (u, "(3x,A,1x,I0,':',1x,A)")  "Insertion mode =", evt%mode, &
         char (evt%get_mode_string ())
  end subroutine evt_isr_write_mode
  
  subroutine evt_isr_write_data (evt, unit, testflag)
    class(evt_isr_t), intent(in) :: evt
    integer, intent(in), optional :: unit
    ...
    case (ISR_PAIR_RECOIL)
       ...
       write (u, "(A)")  "Lorentz boost (partons before ISR c.m. -> lab) ="
  ...
  end subroutine evt_isr_write_data

  subroutine evt_isr_write (evt, unit, verbose, more_verbose, testflag)
    class(evt_isr_t), intent(in) :: evt
    integer, intent(in), optional :: unit
    ...
    if (all (evt%i_beam > 0)) then
       call write_separator (u)
       write (u, "(A,2(1x,I0))")  "Partons before ISR:", evt%i_beam
       do i = 1, 2
    ...
    if (all (evt%i_photon > 0)) then
       call write_separator (u)
       write (u, "(A,2(1x,I0))")  "Radiated photons, collinear:", &
            evt%i_photon
       do i = 1, 2
          call evt%photon(i)%write (u, testflag=testflag)
       end do
       call write_separator (u)
       write (u, "(A)")  "... boosted to c.m.:"
       do i = 1, 2
    ...
    if (all (evt%i_parton > 0)) then
       call write_separator (u)
       write (u, "(A,2(1x,I0))")  "Partons after ISR, collinear:", &
            evt%i_parton
       do i = 1, 2
    ...
    call write_separator (u)
  end subroutine evt_isr_write

  subroutine evt_isr_import_rng (evt, rng)
    class(evt_isr_t), intent(inout) :: evt
    class(rng_t), allocatable, intent(inout) :: rng
    call move_alloc (from = rng, to = evt%rng)
  end subroutine evt_isr_import_rng

  subroutine evt_isr_set_data (evt, sqrts, q_max, m)
    class(evt_isr_t), intent(inout) :: evt
  ...
  end subroutine evt_isr_set_data
    
  subroutine identify_photons (evt)
    class(evt_isr_t), intent(inout) :: evt
    type(flavor_t) :: flv
    integer :: i, k
    k = 2
    ...
    do i = 1, 2
       flv = evt%photon(i)%get_flv ()
       if (flv%get_pdg () /= PHOTON)  call err_photon
    end do
  contains
    subroutine err_count
      call evt%particle_set%write ()
      call msg_fatal ("ISR photon handler: event does not contain two radiated particles")
    end subroutine err_count
    subroutine err_photon
      call evt%particle_set%write ()
      call msg_fatal ("ISR photon handler: event does not contain two radiated photons")
    end subroutine err_photon
  end subroutine identify_photons
  ...
  subroutine identify_partons (evt)
    ...
    if (all (evt%i_photon > 0)) then
       do i = 1, 2
          parent = evt%photon(i)%get_parents ()
          ...
          associate (prt => evt%beam(i))
            ...
            do j = 1, 2
               if (child(j) /= evt%i_photon(i)) then
  ...
  contains
    subroutine err_mismatch
      call evt%particle_set%write ()
      call msg_bug ("ISR photon handler: mismatch in parent-child relations")
  ...
  contains
    subroutine err_non_collinear
      call evt%particle_set%write ()
      call msg_fatal ("ISR photon handler: &
           &partons before ISR are not collinear")
    end subroutine err_non_collinear
  end subroutine boost_to_cm

  subroutine infer_x (evt)
    class(evt_isr_t), intent(inout) :: evt
    real(default) :: E_parent, E_photon, E_parton
    integer :: i
    if (all (evt%i_photon > 0)) then
       do i = 1, 2
          E_parent = energy (evt%pi(i))
          E_photon = energy (evt%ki(i))
          E_parton = energy (evt%qi(i))
          if (E_parent > 0) then
             evt%xc(i) = E_parton / E_parent
             evt%xcb(i)= E_photon / E_parent
          else
             call err_energy
          end if
       end do
  ...
  contains
    subroutine err_energy
      call evt%particle_set%write ()
      call msg_bug ("ISR photon handler: non-positive energy in ISR splitting")
  ...
  subroutine evt_generate_recoil (evt, ok)
    ...
    call generate_recoil (evt%sqrts, evt%q_max, evt%m, evt%xc, evt%xcb, r, &
         evt%km, evt%qm, evt%qo, ok)
  end subroutine evt_generate_recoil

  subroutine replace_photons (evt)
    class(evt_isr_t), intent(inout) :: evt
    integer :: i
    do i = 1, 2
       associate (prt => evt%particle_set%prt(evt%i_photon(i)))
         call prt%set_momentum (evt%lti * evt%km(i))
       end associate
    end do
  end subroutine replace_photons
  ...
  subroutine evt_isr_generate_weighted (evt, probability)
    class(evt_isr_t), intent(inout) :: evt
    ...
    case default
       call msg_bug ("ISR photon handler: generate weighted: unsupported mode")
    end select
    evt%particle_set_exists = .false.
  end subroutine evt_isr_generate_weighted

  subroutine evt_isr_make_particle_set &
    ...
    case (ISR_TRIVIAL_COLLINEAR)
    case (ISR_PAIR_RECOIL)
       call evt%replace_photons ()
       call evt%replace_partons ()
       call evt%transform_outgoing ()
    case default
       call msg_bug ("ISR photon handler: make particle set: unsupported mode")
    end select
    evt%particle_set_exists = .true.
  end subroutine evt_isr_make_particle_set

  subroutine evt_isr_prepare_new_event (evt, i_mci, i_term)
    class(evt_isr_t), intent(inout) :: evt
    integer, intent(in) :: i_mci, i_term
    call evt%reset ()
  end subroutine evt_isr_prepare_new_event


end module isr_photon_handler
