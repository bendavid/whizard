...
module epa_beam_handler
  ...
  integer, parameter, public :: EPA_TRIVIAL_COLLINEAR = 0
  integer, parameter, public :: EPA_PAIR_RECOIL = 1
  

  type, extends (evt_t) :: evt_epa_t
     private
     integer :: mode = EPA_TRIVIAL_COLLINEAR
   ...
   contains
     procedure :: get_mode_string => evt_epa_get_mode_string
     procedure :: set_mode_string => evt_epa_set_mode_string
     procedure :: write_name => evt_epa_write_name
     procedure :: write_mode => evt_epa_write_mode
     procedure :: write_data => evt_epa_write_data
     procedure :: write => evt_epa_write
     procedure :: import_rng => evt_epa_import_rng
     procedure :: set_data => evt_epa_set_data
     ...
     procedure :: generate_weighted => &
          evt_epa_generate_weighted
     procedure :: make_particle_set => &
          evt_epa_make_particle_set
     procedure :: prepare_new_event => &
          evt_epa_prepare_new_event
  end type evt_epa_t


contains

  function evt_epa_get_mode_string (evt) result (string)
    type(string_t) :: string
    class(evt_epa_t), intent(in) :: evt
    select case (evt%mode)
    case (EPA_TRIVIAL_COLLINEAR)
       string = "trivial, collinear"
    case (EPA_PAIR_RECOIL)
       string = "beam-pair recoil"
    case default
       string = "[undefined]"
    end select
  end function evt_epa_get_mode_string

  subroutine evt_epa_set_mode_string (evt, string)
    class(evt_epa_t), intent(inout) :: evt
    type(string_t), intent(in) :: string
    select case (char (string))
    case ("trivial")
       evt%mode = EPA_TRIVIAL_COLLINEAR
    case ("recoil")
       evt%mode = EPA_PAIR_RECOIL
    case default
       call msg_fatal ("EPA handler: mode '" // char (string) &
            // "' is undefined")
    end select
  end subroutine evt_epa_set_mode_string

  subroutine evt_epa_write_name (evt, unit)
    class(evt_epa_t), intent(in) :: evt
    integer, intent(in), optional :: unit
    integer :: u
    u = given_output_unit (unit)
    write (u, "(1x,A)")  "Event transform: EPA beam handler"
  end subroutine evt_epa_write_name

  subroutine evt_epa_write_mode (evt, unit)
    class(evt_epa_t), intent(in) :: evt
    integer, intent(in), optional :: unit
    integer :: u
    u = given_output_unit (unit)
  ...
  end subroutine evt_epa_write_mode
  
  subroutine evt_epa_write_data (evt, unit, testflag)
    class(evt_epa_t), intent(in) :: evt
    integer, intent(in), optional :: unit
    ...
    case (EPA_PAIR_RECOIL)
       ...
       write (u, "(A)")  "Lorentz boost (beams before EPA c.m. -> lab) ="
       call evt%lti%write (u, testflag)
       write (u, "(A)")  "Lorentz transformation (collinear photons &
            &-> photons with recoil in c.m.) ="
       call evt%lto%write (u, testflag)
       write (u, "(A)")  "Combined transformation (photons &
            &-> photons with recoil in lab frame) ="
       call evt%lt%write (u, testflag)
    end select
  end subroutine evt_epa_write_data

  subroutine evt_epa_write (evt, unit, verbose, more_verbose, testflag)
    class(evt_epa_t), intent(in) :: evt
    integer, intent(in), optional :: unit
    ...
    if (all (evt%i_beam > 0)) then
       call write_separator (u)
       write (u, "(A,2(1x,I0))")  "Beams before EPA:", evt%i_beam
       do i = 1, 2
    ...
    if (all (evt%i_radiated > 0)) then
       call write_separator (u)
       write (u, "(A,2(1x,I0))")  "Radiated beam remnants, collinear:", &
            evt%i_radiated
       do i = 1, 2
    ...
    if (all (evt%i_parton > 0)) then
       call write_separator (u)
       write (u, "(A,2(1x,I0))")  "Photons after EPA, collinear:", &
            evt%i_parton
       do i = 1, 2
    ...
    call write_separator (u)
  end subroutine evt_epa_write

  subroutine evt_epa_import_rng (evt, rng)
    class(evt_epa_t), intent(inout) :: evt
    class(rng_t), allocatable, intent(inout) :: rng
    call move_alloc (from = rng, to = evt%rng)
  end subroutine evt_epa_import_rng

  subroutine evt_epa_set_data (evt, sqrts, q_max, m)
    class(evt_epa_t), intent(inout) :: evt
  ...
  end subroutine evt_epa_set_data
  ...
  contains
    subroutine err_count
      call evt%particle_set%write ()
      call msg_fatal ("EPA beam handler: event does not contain two radiated &
           &beam remnants")
  ...
  subroutine identify_partons (evt)
    ...
    if (all (evt%i_radiated > 0)) then
       ...
       do i = 1, 2
          ...
          if (flv%get_pdg () /= PHOTON)  call err_radiated
       end do
    end if
  contains
    subroutine err_mismatch
      call evt%particle_set%write ()
      call msg_bug ("EPA beam handler: mismatch in parent-child relations")
    end subroutine err_mismatch
    subroutine err_radiated
      call evt%particle_set%write ()
      call msg_fatal ("EPA beam handler: partons must be photons")
    end subroutine err_radiated
  ...
  contains
    subroutine err_non_collinear
      call evt%particle_set%write ()
      call msg_fatal ("EPA beam handler: &
           &partons before EPA are not collinear")
  ...
  contains
    subroutine err_energy
      call evt%particle_set%write ()
      call msg_bug ("EPA beam handler: non-positive energy in EPA splitting")
  ...
  subroutine evt_generate_recoil (evt, ok)
    ...
    call generate_recoil (evt%sqrts, evt%q_max, evt%xc * evt%m, evt%xc, evt%xcb, r, &
  ...
  subroutine evt_epa_generate_weighted (evt, probability)
    class(evt_epa_t), intent(inout) :: evt
    ...
    case default
       call msg_bug ("EPA beam handler: generate weighted: unsupported mode")
    end select
    evt%particle_set_exists = .false.
  end subroutine evt_epa_generate_weighted

  subroutine evt_epa_make_particle_set &
    ...
    case default
       call msg_bug ("EPA beam handler: make particle set: unsupported mode")
    end select
    evt%particle_set_exists = .true.
  end subroutine evt_epa_make_particle_set

  subroutine evt_epa_prepare_new_event (evt, i_mci, i_term)
    class(evt_epa_t), intent(inout) :: evt
    integer, intent(in) :: i_mci, i_term
    call evt%reset ()
  end subroutine evt_epa_prepare_new_event


end module epa_beam_handler
