...
module isr_photon_handler_uti
  ...
  use isr_photon_handler, only: evt_isr_t
  ...
  public :: isr_photon_handler_1
  public :: isr_photon_handler_2
  public :: isr_photon_handler_3

contains

  subroutine isr_photon_handler_1 (u)
    integer, intent(in) :: u
    ...
    write (u, "(A)")
    write (u, "(A)")  "* Initialize ISR photon handler transform"
    write (u, "(A)")
    
    evt_trivial%next => evt_isr
    evt_isr%previous => evt_trivial
    
    call evt_isr%write (u)
    
    write (u, "(A)")
    write (u, "(A)")  "* Fill ISR photon handler transform"
    write (u, "(A)")
    
    call evt_isr%prepare_new_event (1, 1)
    call evt_isr%generate_weighted (probability)
    call evt_isr%make_particle_set (0, .false.)
    ...
    write (u, "(A)")
    write (u, "(A)")  "* Test output end: isr_photon_handler_1"

  end subroutine isr_photon_handler_1

  subroutine isr_photon_handler_2 (u)
    integer, intent(in) :: u
    type(os_data_t) :: os_data
    type(particle_set_t) :: pset
    type(model_list_t) :: model_list
    ...
    write (u, "(A)")  "* Test output: isr_photon_handler_2"
    ...
    call evt_isr%set_mode_string (var_str ("recoil"))
    call evt_isr%set_data ( &
         sqrts = sqrts, &
         q_max = sqrts, &
         m = 511.e-3_default &
         )
    ...
    call evt_isr%import_rng (rng)

    call evt_isr%write (u, testflag=.true.)
    ...
    write (u, "(A)")
    write (u, "(A)")  "* Test output end: isr_photon_handler_2"

  end subroutine isr_photon_handler_2

  subroutine isr_photon_handler_3 (u)
    integer, intent(in) :: u
    type(os_data_t) :: os_data
    type(particle_set_t) :: pset
    type(model_list_t) :: model_list
    ...
    write (u, "(A)")  "* Test output: isr_photon_handler_3"
    ...
    write (u, "(A)")
    write (u, "(A)")  "* Test output end: isr_photon_handler_3"

  end subroutine isr_photon_handler_3


end module isr_photon_handler_uti
