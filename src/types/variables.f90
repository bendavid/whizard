...
module variables
  ...
  public :: var_list_replace_string
...
!     var%is_copy = .true.
  ...
  recursive subroutine var_list_final (var_list, follow_link)
    ...
    if (present (follow_link)) then
       if (follow_link) then
          if (associated (var_list%next)) then
...
!           return
  ...
  subroutine var_list_replace (var_list, var, verbose, success)
    type(var_list_t), intent(inout), target :: var_list
    type(var_entry_t), intent(in), target :: var
    type(var_entry_t), pointer :: current_entry, prev_entry, entry_save
    type(var_list_t), pointer :: current_list
    logical, intent(in), optional :: verbose
    logical, intent(out) :: success
    type(string_t) :: name_to_search
    name_to_search = var%name
    allocate (prev_entry, entry_save)
    current_list => var_list
    do 
      if (associated (current_list%first)) then
        current_entry => current_list%first
        do
          if (current_entry%name == name_to_search) then
            entry_save = current_entry
            current_entry = var
            current_entry%next => entry_save%next
            prev_entry%next => current_entry
            nullify (entry_save)
            success = .true.
            exit
          else
            if (associated (current_entry%next)) then
        ...
        if (success) exit
        if (associated (current_list%next)) then
  ...
  end subroutine var_list_replace

   subroutine var_list_replace_string &
       (var_list, name, sval, locked, verbose, intrinsic, user, success)
    type(var_list_t), intent(inout) :: var_list
    type(string_t), intent(in) :: name
    type(string_t), intent(in), optional :: sval
    logical, intent(in), optional :: locked, verbose, intrinsic, user
    ...
    call var_entry_init_string (var, name, sval, intrinsic, user)
    if (present (locked)) call var_entry_lock (var, locked)
    call var_list_replace (var_list, var, verbose, success)
  end subroutine var_list_replace_string
