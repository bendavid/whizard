...
!! Tell the caller that this is not the true HepMC3 library
logical(c_bool) function hepmc3_available () bind(C)
  use iso_c_binding
  hepmc3_available = .false.
end function hepmc3_available
...
function gen_vertex_is_valid (v_obj) result (flag) bind(C)
  ...
  flag = .false.
  write (0, "(A)")  "*************************************************************"
  write (0, "(A)")  "*** HepM3C: Error: library not linked, WHIZARD terminates ***"
  write (0, "(A)")  "*************************************************************"
  stop
