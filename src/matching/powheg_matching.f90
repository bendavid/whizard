...
module powheg_matching
  ...
  use nlo_data, only: phs_point_set_t, phs_identifier_t
  use phs_fks
  use matching_base
...
contains
  ...
  subroutine sudakov_massive_fsr_generate_xi_and_y_and_phi (sudakov, r)
    ...
    i_phs = sudakov%process_deps%alr_to_i_phs (r%alr)
    q0 = sqrt (sudakov%event_deps%s_hat)
  ...
  function powheg_matching_compute_sqme_real (powheg, alr, alpha_s) result (sqme)
    ...
    class is (pcm_instance_nlo_t)
       ...
       associate (instance => powheg%process_instance)
          call instance%compute_sqme_real_rad (i_phs, 1, alpha_s)
          call instance%apply_fks_factors_for_powheg (i_phs)
       end associate
       !!!sqme = pcm%collector%sqme_real_per_phs (1, i_phs)
       sqme = pcm%collector%current_sqme_real
    end select
  ...
  subroutine powheg_matching_setup_nlo_environment (matching)
    ...
    class is (pcm_instance_nlo_t)
       ...
       i_real = matching%process_instance%get_associated_real ()
       associate (process_deps => matching%process_deps)
          ...
          associate (nlo_controller => matching%process_instance%component(i_real)%nlo_controller)
             allocate (matching%process_deps%alr_to_i_phs &
                (size (nlo_controller%real_kinematics%alr_to_i_phs)))
             process_deps%alr_to_i_phs = nlo_controller%real_kinematics%alr_to_i_phs
          end associate
       end associate
       call matching%event_deps%p_born_lab%init (n_in + n_out_born, 1)
       call matching%event_deps%p_born_cms%init (n_in + n_out_born, 1)
       call matching%event_deps%p_real_lab%init (n_in + n_out_real, n_phs)
       call matching%event_deps%p_real_cms%init (n_in + n_out_real, n_phs)
    end select
  ...
  subroutine powheg_matching_copy_momenta (matching)
     ...
     class is (pcm_instance_nlo_t)
        ...
        do i_phs = 1, matching%event_deps%p_real_cms%get_n_phs ()
           call pcm%controller%real_kinematics%p_real_cms%set_momenta &
              (i_phs, matching%event_deps%p_real_cms%get_momenta (i_phs))
           call pcm%controller%real_kinematics%p_real_lab%set_momenta &
         ...
         associate (fks => powheg%phs_fks_generator)
           do i_call = 1, n_calls1
              do alr = 1, powheg%process_deps%n_alr
                 ...
                 ! TODO: (cw 2015-12-29) Use correct i_phs and phs_identifiers
                 !!! powheg%event_deps%p_real_cms = fks%generate_fsr_from_x &
                 !!!   (random, emitter, i_phs, p_born)
