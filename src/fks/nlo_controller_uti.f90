...
module nlo_controller_uti
  use nlo_controller, only: color_data_t
  use nlo_data, only: NO_FACTORIZATION
...
contains
  ...
  subroutine nlo_color_data_1 (u)
   ...
   allocate (ghost_flag_born (n_legs_born, n_col_born))
   allocate (ghost_flag_real (n_legs_real, n_col_real))
   ghost_flag_born (:, 1) = .false.
   ghost_flag_real (:, 1) = .false.
   ghost_flag_real (1 : n_legs_born, 2) = .false.
   ghost_flag_real (n_legs_real, 2) = .true.
   call color_data%init_ghost_flags (ghost_flag_born, ghost_flag_real)
   allocate (included_color_structures (2))
   included_color_structures = [1, 2]
   call color_data%init_color (reg_data, included_color_structures)
   call color_data%check_equivalences ()
   call color_data%init_betaij (reg_data, NO_FACTORIZATION, included_color_structures)
  ...
  subroutine nlo_color_data_2 (u)
   ...
   allocate (ghost_flag_real (n_legs_real, n_col_real))
   ghost_flag_born (:, 1) = .false.
   ghost_flag_real (1, 1) = .true.
   ghost_flag_real (2 : n_legs_real, 1) = .false.
   ghost_flag_real (1 : n_legs_born, 2) = .false.
   ghost_flag_real (n_legs_real, 2) = .true.
   ghost_flag_real (:, 3) = .false.
   ghost_flag_real (:, 4) = .false.
   ghost_flag_real (1, 5) = .false.
   ghost_flag_real (2, 5) = .false.
   ghost_flag_real (3 : n_legs_real, 5) = .true.
   ghost_flag_real (:, 6) = .false.
   call color_data%init_ghost_flags (ghost_flag_born, ghost_flag_real)
   allocate (included_color_structures (2))
   included_color_structures = [2, 6]
   call color_data%init_color (reg_data, included_color_structures)
   call color_data%check_equivalences ()
   call color_data%init_betaij (reg_data, NO_FACTORIZATION, included_color_structures)
   call color_data%write (u, verbose = .true.)
  ...
  subroutine nlo_color_data_3 (u)
   ...
   allocate (ghost_flag_real (n_legs_real, n_col_real))
   do i_col = 1, n_col_born
      ghost_flag_born (:, i_col) = .false.
   end do
   do i_col = 1, n_col_real, 6
      ghost_flag_real (1, i_col) = .true.
      ghost_flag_real (2 : n_legs_real, i_col) = .false.
      ghost_flag_real (1 : n_legs_born, i_col + 1) = .false.
      ghost_flag_real (n_legs_real, i_col + 1) = .true.
      ghost_flag_real (:, i_col + 2) = .false.
      ghost_flag_real (:, i_col + 3) = .false.
      ghost_flag_real (1, i_col + 4) = .false.
      ghost_flag_real (2, i_col + 4) = .false.
      ghost_flag_real (3 : n_legs_real, i_col + 4) = .true.
      ghost_flag_real (:, i_col + 5) = .false.
   end do
   call color_data%init_ghost_flags (ghost_flag_born, ghost_flag_real)
   allocate (included_color_structures (2))
   included_color_structures = [2, 6]
