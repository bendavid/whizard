...
contains
  ...
  subroutine mlm_matching_settings_write(mlm_matching_settings, unit)
    ...
    write (u, *) "mlm_Emin                     = ", mlm_matching_settings%mlm_Rmin
    write (u, *) "mlm_nmaxMEjets               = ", mlm_matching_settings%mlm_nmaxMEjets
    write (u, *)
    write (u, *) "mlm_ETclusfactor  (D=0.2)    = ", mlm_matching_settings%mlm_ETclusfactor
    write (u, *) "mlm_ETclusminE    (D=5.0)    = ", mlm_matching_settings%mlm_ETclusminE
    write (u, *) "mlm_etaclusfactor (D=1.0)    = ", mlm_matching_settings%mlm_etaClusfactor
    write (u, *) "mlm_Rclusfactor   (D=1.0)    = ", mlm_matching_settings%mlm_RClusfactor
    write (u, *) "mlm_Eclusfactor   (D=1.0)    = ", mlm_matching_settings%mlm_RClusfactor
  end subroutine mlm_matching_settings_write

  subroutine mlm_matching_data_write(mlm_matching_data, unit)
    type(mlm_matching_data_t), intent(in) :: mlm_matching_data
    integer, intent(in), optional :: unit
    integer :: i
    integer :: u
    u = output_unit (unit);  if (u < 0)  return
    ...
    if(allocated(mlm_matching_data%JETS_ME)) then
       do i=1, size(mlm_matching_data%JETS_ME)
          call vector4_write(mlm_matching_data%JETS_ME(i), unit=u)
       end do
    end if
    write(u, *) "================"
    write(u, *) "P_PS"
    if(allocated(mlm_matching_data%P_PS)) then
       do i=1, size(mlm_matching_data%P_PS)
          call vector4_write(mlm_matching_data%P_PS(i), unit=u)
       end do
    end if
    write(u, *) "================"
    write(u, *) "JETS_PS"
    if(allocated(mlm_matching_data%JETS_PS)) then
       do i=1, size(mlm_matching_data%JETS_PS)
          call vector4_write(mlm_matching_data%JETS_PS(i), unit=u)
       end do
    end if
    write(u, *) "================"
 ...
 !   if(allocated(mlm_matching_data%P_ME)) deallocate(mlm_matching_data%P_ME)
    if(allocated(mlm_matching_data%P_PS)) deallocate(mlm_matching_data%P_PS)
    if(allocated(mlm_matching_data%JETS_ME)) deallocate(mlm_matching_data%JETS_ME)
    if(allocated(mlm_matching_data%JETS_PS)) deallocate(mlm_matching_data%JETS_PS)
  ...
  subroutine mlm_matching(mlm_matching_data, mlm_matching_settings, vetoed)
    ...
    integer :: NJET, NSUB
    integer :: imode
    real(kind=double) :: ECUT = 1._double
    integer :: ip1,ip2
    ...
    if(n_jets_PS>0) then
       ...
       do i=1, n_jets_ps
          ...
          PP(2,i) = vector4_get_component(mlm_matching_data%p_PS(i), 2)
          PP(3,i) = vector4_get_component(mlm_matching_data%p_PS(i), 3)
          PP(4,i) = vector4_get_component(mlm_matching_data%p_PS(i), 0)
       end do
       ...
       call KTCLUR(imode, PP, n_jets_ps, mlm_matching_settings%mlm_Rclusfactor*mlm_matching_settings%mlm_Rmin, ECUT, y, *999)
...
!!$         !call fastjetsiscone(PP,n,R,f,P_JETS,NJET)      ! SiSCone-Algorithm
    ...
    if(n_jets_ME > 0 .and. n_jets_PS > 0) then
       ...
       do i=1, n_jets_ps
          ...
          PP(4,i) = vector4_get_component(mlm_matching_data%JETS_PS(i), 0)
       end do
