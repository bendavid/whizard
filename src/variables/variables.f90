...
!           intrinsic=.true.)
    ...
    call var_list%append_real (var_str ("resonance_on_shell_limit"), 4._default, &
          intrinsic=.true., &
          description=var_str ( &
          'The real variable \texttt{resonance\_on\_shell\_limit ' // &
          '= {\em <num>}} specifies the relative distance from a ' // &
          'resonance peak, such that the kinematical configuration ' // &
          'can be considered on-shell.  This is relevant only if ' // &
    ...
    call var_list%append_log (var_str ("?recover_beams"), .true., &
          intrinsic=.true., &
          description=var_str ('Flag that decides whehter the beam particles ' // &
